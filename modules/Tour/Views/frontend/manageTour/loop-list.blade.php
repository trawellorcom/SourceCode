        ...
        <div class="col-md-9">
            ...
            <div class="control-action">
                ...
                @if(Auth::user()->hasPermissionTo('tour_update'))
                    <a href="{{url(app_get_locale()."/user/tour/edit/".$row->id)}}" class="btn btn-warning">{{__("Edit")}}</a>
                @endif
                @if(Auth::user()->hasPermissionTo('tour_delete'))
                    <a href="{{url(app_get_locale()."/user/tour/del/".$row->id)}}" class="btn btn-danger" data-confirm="<?php echo e(__("Do you want to delete?")); ?>">{{__("Del")}}</a>
                @endif
            </div>
        </div>
    </div>
