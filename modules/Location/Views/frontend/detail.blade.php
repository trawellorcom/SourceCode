                    ...
                    <div class="g-location-module py-5 border-top border-bottom">
                        <div class="row">
                            <div class="col-12 col-lg-5">
                                <h3>{{__("Explore the City")}}</h3>
                            </div>
                            <div class="col-12 col-lg-7">
                                <ul class="location-module-nav nav nav-pills justify-content-lg-end">
                                    @php($i=0)
                                    @foreach($types as $type=>$moduleClass)
                                        @php($data[$type] = (new $moduleClass)->where('location_id',$row->id)->where('status','publish')->with('location')->take(8)->get())
                                        @if($data[$type]->count()>0)
                                            <li>
                                                <a class="{{$i==0?'active':""}}" href="#module-{{$type}}" data-toggle="tab">{{__(ucfirst($type))}}</a>
                                            </li>
                                            @php($i++)
                                        @endif
                                    @endforeach
                                ...
                                @if(view()->exists($view))
                                    @if($data[$type]->count()>0)
                                        <div class="tab-pane {{$i==0?'active':""}}" id="module-{{$type}}">
                                            ...
                                            <p class="text-center"><a class="btn btn-primary btn-search" href="{{$row->getLinkForPageSearch($type)}}">View More</a></p>
                                        </div>
                                        @php($i++)
                                    @endif
                                @endif
